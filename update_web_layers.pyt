...
class Tool(object):
    ...
    def execute(self, parameters, messages):
        ...
        def custom_osm_fields(input_path,fields):
            ...
            new_name = f'{os.path.basename(input_path)[:-19]}_customfields'
            feature_path = os.path.join(os.path.dirname(input_path),f'{new_name}.json')
            with open(feature_path, 'w') as file:
                json.dump(python_data,file,indent=4)
            arcpy.conversion.JSONToFeatures(feature_path,new_name)
...
def calculate_code(highway,cycleway,cycleway_left,cycleway_right,cycleway_both,bicycle,name):
    if highway=='cycleway' or cycleway=='track' or cycleway_left=='track' or cycleway_right=='track' or cycleway_both=='track' or cycleway_right=='separate':
        ...
        return 'Trail'
        '''
            arcpy.management.AddField('bikelanes_customfields','code','TEXT')
            arcpy.management.CalculateField('bikelanes_customfields','code',expression,'PYTHON3',codeblock)
...
def calculate_desc(code):
    if code=='NYC DOT Class I':
        ...
        return 'Trail bicycle path'
            '''
            arcpy.management.AddField('bikelanes_customfields','descriptor','TEXT')
            arcpy.management.CalculateField('bikelanes_customfields','descriptor',expression2,'PYTHON3',codeblock2)

            # Bike lane symbology
            bikelane_lyr = active_map.listLayers("bikelanes_customfields")[0]
